...
const blockExplorerUrl = computed(() => {
  return (
    networkInfo[tokenData.chainId].blockExplorer +
    "address/" +
    tokenData.address
  );
});
...
const hasSetMinter = computed(() => {
  return tokenData.minter != "0x0000000000000000000000000000000000000000";
});
const adminRole =
...
async function mintToken() {
  ...
  try {
    await token.mint(BigNumber.from(tokenData.nextMintAllowance));
